...
import { DriveFile, DrivePermission } from '../types/index.js';
...
class GoogleDriveMCPServer {
      ...
      try {
        switch (name) {
          ...
          case 'get_permissions':
            return await this.getPermissions(args as any);
          case 'update_permissions':
            return await this.updatePermissions(args as any);
          default:
  ...
  private async listFiles(args: { folderId?: string; pageSize?: number; orderBy?: string }) {
    ...
    const driveFiles: DriveFile[] = files.map((file: any) => this.parseDriveFile(file));
  ...
  private async searchFiles(args: { query: string; mimeType?: string; pageSize?: number }) {
  ...
  private async createFolder(args: { name: string; parentId?: string }) {
  ...
  private async uploadFile(args: { name: string; content: string; mimeType: string; parentId?: string }) {
  ...
  private async deleteFile(args: { fileId: string }) {
  ...
  private async shareFile(args: { fileId: string; email: string; role: string }) {
  ...
  private async downloadFile(args: { fileId: string }) {
    const response = await this.drive.files.get({
      fileId: args.fileId,
      alt: 'media',
    });
  ...
  private async moveFile(args: { fileId: string; newParentId: string }) {
  ...
  private async copyFile(args: { fileId: string; name: string; parentId?: string }) {
    const { fileId, name, parentId } = args;

    const resource: any = { name };
    if (parentId) {
  ...
  private async getPermissions(args: { fileId: string }) {
  ...
  private async updatePermissions(args: { fileId: string; permissionId: string; role: string }) {
  ...
  private parseDriveFile(data: any): DriveFile {
    return {
      id: data.id,
      name: data.name,
