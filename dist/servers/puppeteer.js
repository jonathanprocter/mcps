...
class PuppeteerMCPServer {
    ...
    async scrapePage(args) {
        const { url, selector, waitForSelector, includeLinks = false, includeImages = false, timeout = 30000, } = args;
    ...
    async takeScreenshot(args) {
        const { url, selector, fullPage = false, width = 1920, height = 1080, format = 'png', quality = 80, } = args;
        const browser = await this.ensureBrowser();
        const page = await browser.newPage();
        try {
    ...
    async generatePDF(args) {
        const { url, format = 'A4', landscape = false, printBackground = false, marginTop = '0', marginRight = '0', marginBottom = '0', marginLeft = '0', } = args;
        const browser = await this.ensureBrowser();
        const page = await browser.newPage();
        try {
            await page.goto(url);
