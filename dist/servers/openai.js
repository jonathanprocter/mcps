...
class OpenAIMCPServer {
    ...
    async chatCompletion(args) {
        const { messages, model = 'gpt-4o', // the newest OpenAI model is "gpt-4o" which was released May 13, 2024. do not change this unless explicitly requested by the user
        temperature, maxTokens, topP, frequencyPenalty, presencePenalty, stop, stream = false, } = args;
    ...
    async generateImage(args) {
        const { prompt, model = 'dall-e-3', n = 1, size = '1024x1024', quality = 'standard', style = 'vivid', responseFormat = 'url', } = args;
    ...
    async editImage(args) {
        const { image, mask, prompt, n = 1, size = '1024x1024', responseFormat = 'url', } = args;
    ...
    async textToSpeech(args) {
        const { input, model = 'tts-1', voice = 'alloy', responseFormat = 'mp3', speed = 1.0, } = args;
    ...
    async moderateContent(args) {
        const { input, model = 'text-moderation-latest', } = args;
